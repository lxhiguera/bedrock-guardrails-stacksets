...
- `bootstrap/github-oidc-role.yaml` — optional: `github-actions-validate-role` (read-only, `cloudformation:ValidateTemplate` only), trust-scoped via `sub: repo:<org>/<repo>:*` to this repo, for the optional `aws-validate` job in `pr-validate.yml`. Has a `CreateOidcProvider` toggle since an account can only have one `token.actions.githubusercontent.com` OIDC provider — set it to `false` if one already exists (e.g. from another pipeline).
